...
namespace ValheimRecipePinner
{
    public class ContainerScanner
    {
        ...
        // Prevents concurrent or spam-toggle re-initializations
        private static volatile bool _isInitializing = false;

        public void InitializeContainers()
        {
            // Guard: don't track anything while scanning is disabled
            ...
            // Guard: prevent concurrent or rapid re-entry (spam-toggle protection)
            if (_isInitializing)
            {
            ...
            try
            {
                ...
                lock (ContainerLock)
                {
                    // List is expected to be empty here (ClearAll was called before this on re-enable)
        ...
        /// <summary>
        /// Clears all tracked container references and resets the set.
        /// Call before re-initializing (e.g. when re-enabling chest scanning at runtime).
        /// </summary>
        ...
        public void UpdateScanning()
        {
            ...
            bool isContainerOpen = InventoryGui.instance != null &&
                ReflectionHelper.GetCurrentContainer(InventoryGui.instance) != null;
        ...
        public static void TrackContainerAwake(Container __instance)
        {
            ...
            if (!RecipePinnerPlugin.EnableChestScanning.Value) return; // Don't track if scanning disabled
    ...
    public class ContainerTracker : MonoBehaviour
    {
        ...
        {
            // Always clean up regardless of scan setting to prevent leaks
