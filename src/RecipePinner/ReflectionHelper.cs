...
namespace ValheimRecipePinner
{
    public static class ReflectionHelper
    {
        ...
        private static FieldInfo[] _blockingInventoryPanelFields;
        ...
        public static bool IsBlockingInventoryPanelOpen(InventoryGui gui)
        {
            if (gui == null) return false;

            foreach (FieldInfo field in GetBlockingInventoryPanelFields())
            {
                object value = field.GetValue(gui);
                if (IsActiveUnityObject(value))
                    return true;
            }
        ...
        private static FieldInfo[] GetBlockingInventoryPanelFields()
        {
            if (_blockingInventoryPanelFields != null)
                return _blockingInventoryPanelFields;

            var fields = new System.Collections.Generic.List<FieldInfo>();
            foreach (FieldInfo field in typeof(InventoryGui).GetFields(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic))
            {
                string name = field.Name.ToLowerInvariant();
                if (!name.Contains("troph") &&
                    !name.Contains("skill") &&
            ...
            _blockingInventoryPanelFields = fields.ToArray();
            return _blockingInventoryPanelFields;
        }

        private static bool IsActiveUnityObject(object value)
        {
    ...
    public static class InputHelper
    {
        ...
        public static bool IsMouseOverRect(RectTransform rect, bool logHit = true)
        {
            if (rect == null)
            {
